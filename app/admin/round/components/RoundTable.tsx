...
import { type Column } from "@/app/types/table/column.data";
import { type Row } from "@/app/types/table/row.data";
...
export default function RoundTable({ selectedEvent }: RoundTableProps) {
  const { data: rounds } = useAdminFetch<Round[]>(
    `/api/admin/round?eventId=${selectedEvent.id}`
  );
