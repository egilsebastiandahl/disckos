...
interface PlayersInRoundChooserProps {
  ...
  updateActivePlayers: (players: SimplePlayer[]) => void;
}

export default function PlayersInRoundChooser({
  selectedEvent,
  activePlayers,
  updateActivePlayers,
}: PlayersInRoundChooserProps) {
  ...
  const handleActivePlayersChange = (playerId: any) => {
    ...
    if (isActive) {
      updateActivePlayers(activePlayers.filter((p) => p.id !== playerId));
    } else {
      updateActivePlayers([...activePlayers, { id: player.id, name: player.name }]);
    }
  };
  ...
  const onPlayerSelect = (value: string) => {
    ...
    if (isActive) {
      updateActivePlayers(activePlayers.filter((p) => p.id !== value));
    } else {
      updateActivePlayers([...activePlayers, { id: player.id, name: player.name }]);
    }
  };
  ...
  return (
    <div className="w-full">
      ...
      <ul className="list-disc list-inside">{activePlayers.map((player) => player.name).join(", ")}</ul>
    </div>
  );
  // return (
  //     <Combobox
