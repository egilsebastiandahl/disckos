  ...
  return (
    <Drawer direction="bottom" onOpenChange={setIsOpen} open={isOpen}>
      <DrawerTrigger className="hover:cursor-pointer">
        <div className="flex justify-center w-full bg-primary text-primary-foreground border border-transparent hover:bg-primary/85 p-2 rounded-xl shadow-sm transition">
          <span className="font-bold">NY RUNDE &nbsp;</span>
          <PlusIcon />
        </div>
      </DrawerTrigger>

      {selectedRound ? (
        <RoundDrawerUpdate currentRound={selectedRound} selectedEvent={selectedEvent} />
      ) : (
        <RoundDrawerCreate selectedEvent={selectedEvent} />
      )}
    </Drawer>
  );
}
