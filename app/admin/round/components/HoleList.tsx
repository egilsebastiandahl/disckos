...
export default function HoleList({ holes, setHoles, activePlayers }: HoleListProps) {
    ...
    const setPlayerScoreForHole = (holeNumber: number, playerId: string, throws: number) => {
        ...
        if (existingPlayerScore) {
            updatedPlayerScores = holeToUpdate.playerScores!.map(ps => ps.playerId === playerId ? { playerId, throws } : ps);
    ...
    return (
        <div className="flex gap-4 flex-wrap">
            {holes.map((hole, index) => (
                <HoleListItem key={index} hole={hole} removeHole={removeHole} updateHole={updateHole} onEditHoleClick={hole => onEditHoleClick(hole)} />
            ))}
            <HoleDrawer isOpen={isDrawerOpen} setIsOpen={setIsDrawerOpen} activePlayers={activePlayers} hole={holeBeingEdited} setPlayerScoreForHole={setPlayerScoreForHole} />
